...
exports.request = function(args) {
    if (!Alloy.Globals.isOnline()) {
        Alloy.Globals.toast.show('You do not have internet access.');
        ...
        onload: function(response) {
            ...
            if (this.status == 200) {
                ...
                try {
                    ...
                    Alloy.Globals.toast.show("There was an error communicating with the server");
...
exports.set = function(parameter, value) {
    try {
        ...
        Alloy.Globals.toast.show(
            "You are trying to set a parameter that doesn't exist");
        return false;
    }
};
...
exports.get = function(parameter) {
    try {
        ...
        Alloy.Globals.toast.show(
            "You are trying to get a parameter that doesn't exist");
        return false;
    }
};
...
function globalSuccessCallback(response) {
    Alloy.Globals.toast.show("Success Response: " + JSON.stringify(response,
        null, 4));
}
...
function globalErrorCallback(err, context) {
    Alloy.Globals.toast.show("Error err: " + JSON.stringify(err));
    Alloy.Globals.toast.show("Error context: " + JSON.stringify(context, null, 4));
}
